...
public abstract class State implements AutoCloseable {
   ...
   protected State(StateInitializer init) {
      ...
      this.path = init.path;
      this.input = init instanceof Parser
            ? ((Parser) init).in
            : null;
   ...
   public final boolean contains(Obj ref) {
      ...
      try {
         ...
         if (0 < ID)
            return (null != p.data) && ref == p.data[ID - 1];
    ...
    * @throws OGSSException if an inconsistency is found
    */
      ...
      for (Pool<?> p : classes)
         for (FieldDeclaration<?, ?> f : p.dataFields)
            try {
               ...
               throw new OGSSException(String.format("check failed in %s.%s:\n  %s", p.name, f.name, e.getMessage()),
                     e);
            }
   }
    ...
    * @throws OGSSException if check fails
    */
